...
function Required(target: any, name: string) {
  ...
  console.log('Required name', name);
...
function PositiveNumber(target: any, name: string) {
  console.log('PositiveNumber target', target);
  console.log('PositiveNumber name', name);
  registeredValidators[target.constructor.name] = {
    ...registeredValidators[target.constructor.name],
    [name]: [
      ...(registeredValidators[target.constructor.name]?.[name] ?? []),
...
function validate(obj: any) {
  ...
  let isValid = true;
  for (const prop in objValidatorConfig) { //prop {title, price}
...
courseForm.addEventListener('submit', (event) => {
  ...
  console.log('validating newCourse', newCourse);
